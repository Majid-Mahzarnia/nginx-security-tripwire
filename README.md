...
## Resetting dashboard history

Version 1.1.2 includes `bin/reset-security-dashboard.sh` for safely clearing
dashboard event history. The utility creates a timestamped backup first,
clears only `candidates.log`, then rebuilds attacker data and the dashboard.

Example:

```bash
sudo SITE=example.com PREFIX=example ./bin/reset-security-dashboard.sh
```

Adjust `SITE`, `PREFIX`, or `BASE` to match the installed instance.
